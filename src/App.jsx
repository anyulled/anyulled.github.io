...
import { describeTechnology } from './technologyDescriptions.js';
...
const championSections = [
  ...
  { to: '/java-champion/nomination', label: 'Nomination draft' },
];
...
function NominationPage({ champion }) {
  const nomination = champion.nomination;
  return <section className="section nomination-section"><div className="nomination-actions"><a className="secondary-action" href={nomination.documentPath} download>Download PDF</a><button className="secondary-action" type="button" onClick={() => window.print()}>Print nomination</button></div><div className="nomination-paper"><span className="eyebrow">Java Champion nomination</span><h2>{nomination.title}</h2><p className="nomination-subtitle">{nomination.subtitle}</p><div className="nomination-meta">{nomination.metadata.map((item) => <div key={item.label}><strong>{item.label}</strong>{item.value.startsWith('http') ? <a href={item.value} target="_blank" rel="noreferrer">{item.value}</a> : <span>{item.value}</span>}</div>)}</div>{nomination.sections.map((section) => <section key={section.title} className="nomination-section-block"><h3>{section.title}</h3>{section.paragraphs?.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}{section.subsections?.map((subsection) => <article key={subsection.title} className="nomination-subsection"><h4>{subsection.title}</h4>{subsection.paragraphs?.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}<ExternalLinks links={subsection.links} /></article>)}</section>)}</div></section>;
}
...
function AppContent() {
  ...
  return <main className="app-shell"><Routes><Route path="/" element={<HomePage profile={profile} />} /><Route path="/java-champion" element={<JavaChampionLayout profile={profile} />}><Route index element={<ChampionOverview champion={profile.javaChampion} />} /><Route path="memberships" element={<MembershipsPage champion={profile.javaChampion} />} /><Route path="events" element={<EventsPage champion={profile.javaChampion} />} /><Route path="talks" element={<TalksPage champion={profile.javaChampion} />} /><Route path="projects" element={<ProjectsPage champion={profile.javaChampion} />} /><Route path="open-source" element={<OpenSourcePage champion={profile.javaChampion} />} /><Route path="nomination" element={<NominationPage champion={profile.javaChampion} />} /></Route></Routes></main>;
}
